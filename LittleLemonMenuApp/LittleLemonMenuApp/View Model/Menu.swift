...
struct MyToggleStyle: ToggleStyle {
    func makeBody(configuration: Configuration) -> some View {
        Button {
            ...
            ZStack {
                RoundedRectangle(cornerRadius: 8).fill(Color("llwhite"))
                configuration.label
            }
            .font(.custom("Karla-Regular_Bold", size: 20))
            ...
            VStack {
                ...
                HStack{
                    Text("ORDER FOR DELIVERY!")
                        .font(.custom("Karla-Regular_ExtraBold", size: 20))
                        .foregroundColor(Color("llblack"))
                        .frame(alignment: .leading)

                    Image("Delivery van").resizable().frame(width: 45, height:35)
                    Spacer()
                }
                .padding(.top)
                .padding(.leading)
                ...
                FetchedObjects(predicate: buildPredicate(),
                    ...
                    (dishes: [Dish]) in
                    List(dishes) { dish in
                        NavigationLink(destination: FoodView(dish: dish)) {
                            FoodRow(dish: dish)
                        }
                    }
                    .listStyle(.plain)
